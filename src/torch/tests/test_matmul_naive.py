...
if __name__ == '__main__':
    ...
    _matmul_naive = torch.ops.j6q_cu_ext.matmul_naive

    for args in samples:
        opcheck(_matmul_naive.default, args)

        result = _matmul_naive(*args)
        ...
        args = [a.to(torch.double) for a in args]
        result = _matmul_naive(*args)
        result = result.mean()
        result.backward()
        gradcheck(_matmul_naive, args, eps=1e-6)